...
if [[ -z "$OUTPUT_FILE" ]]; then
  ...
  mkdir -p "$OUTPUT_DIR"
  OUTPUT_FILE="$OUTPUT_DIR/staging-$(date -u +%Y%m%dT%H%M%SZ).md"
fi
...
{
  echo "# Staging deployment record"
  echo
  ...
  echo "## Services"
  echo
  echo "- Storefront active/enabled: $(systemctl is-active "ofisnye-dveri@staging.service" 2>/dev/null || true) / $(systemctl is-enabled "ofisnye-dveri@staging.service" 2>/dev/null || true)"
  ...
  echo "- Backup timer active/enabled: $(systemctl is-active wordpress-backup.timer 2>/dev/null || true) / $(systemctl is-enabled wordpress-backup.timer 2>/dev/null || true)"
  echo
  echo "## TLS storefront"
  echo '```text'
...
echo "Staging state recorded: $OUTPUT_FILE"
