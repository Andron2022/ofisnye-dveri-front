...
required=(
  ENVIRONMENT PROJECT_NAME STOREFRONT_DOMAIN WORDPRESS_DOMAIN VDS_IPV4 DNS_PROVIDER DNS_TTL
...
[[ "${ENVIRONMENT:-}" == "staging" || "${ENVIRONMENT:-}" == "production" ]] || errors+=("ENVIRONMENT must be staging or production")
...
if [[ "${STOREFRONT_PORT:-0}" =~ ^[0-9]+$ ]] && (( STOREFRONT_PORT < 1024 || STOREFRONT_PORT > 65535 )); then
  errors+=("STOREFRONT_PORT must be between 1024 and 65535")
fi

if [[ "${ENVIRONMENT:-}" == "staging" ]]; then
  [[ "${STOREFRONT_PORT:-}" == "3001" ]] || errors+=("Staging STOREFRONT_PORT must be 3001")
fi

if [[ "${ENVIRONMENT:-}" == "production" ]]; then
  [[ "${STOREFRONT_PORT:-}" == "3000" ]] || errors+=("Production STOREFRONT_PORT must be 3000")
  [[ "${GIT_DEPLOY_REF:-}" == "origin/main" ]] || errors+=("Production GIT_DEPLOY_REF must be origin/main")
  [[ -n "${STOREFRONT_WWW_DOMAIN:-}" ]] || errors+=("STOREFRONT_WWW_DOMAIN is required for production")
  [[ -n "${WORDPRESS_ADMIN_BASIC_AUTH_FILE:-}" ]] || errors+=("WORDPRESS_ADMIN_BASIC_AUTH_FILE is required for production")
fi

[[ "${APP_ROOT:-}" == "${APP_BASE:-}/${ENVIRONMENT:-}" ]] || errors+=("APP_ROOT must be APP_BASE/ENVIRONMENT")
[[ "${REPOSITORY_DIR:-}" == "${APP_BASE:-}/repository" ]] || errors+=("REPOSITORY_DIR must be APP_BASE/repository")
[[ "${STOREFRONT_ENV_FILE:-}" == "/etc/ofisnye-dveri/${ENVIRONMENT:-}.env" ]] || errors+=("STOREFRONT_ENV_FILE must match ENVIRONMENT")

if ((${#errors[@]})); then
  printf '%s inventory validation failed:\n' "${ENVIRONMENT:-Unknown}" >&2
  printf -- '- %s\n' "${errors[@]}" >&2
  exit 1
fi

printf '%s inventory is valid: storefront=%s wordpress=%s vds=%s\n' \
  "$ENVIRONMENT" "$STOREFRONT_DOMAIN" "$WORDPRESS_DOMAIN" "$VDS_IPV4"
